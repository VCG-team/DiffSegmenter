...
def load_image_label_list_from_npy(img_name_list, label_file_path=None):
    if label_file_path is None:
        label_file_path = '/root/autodl-tmp/wjl/ptp_diffusion/voc12/cls_labels.npy'
...
class VOC12Dataset(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, transform=None, gen_attn=False):
        ...
        self.img_name_list,self.img_id = load_img_name_list_100(img_name_list_path)
        # else:
        #     img_name_list_path = os.path.join(img_name_list_path, f'{"train_100" if train or gen_attn else "val"}_id.txt')
        #     self.img_name_list= load_img_name_list(img_name_list_path)
...
class VOC12Dataset_seg(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, transform=None, gen_attn=False,is_big_data=True):
        if not train :
...
####分割重写数据集
class VOC12Dataset_1wseg(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, transform=None, gen_attn=False):
        # if train:
...
class VOC12DatasetMS(Dataset):
    def __init__(self, img_name_list_path, voc12_root, scales, train=True, transform=None, gen_attn=False, unit=1):
        img_name_list_path = os.path.join(img_name_list_path, f'{"train_100" if train or gen_attn else "val"}_id.txt')
        self.img_name_list = load_img_name_list_100(img_name_list_path)
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
        self.voc12_root = voc12_root
        self.transform = transform
        self.unit = unit
...
def build_dataset(is_train, args, gen_attn=False, is_big_data=True):
    ...
    if args.data_set == 'VOC12':
        ...
        dataset = VOC12Dataset_seg(img_name_list_path=args.img_list, voc12_root=args.data_path,
