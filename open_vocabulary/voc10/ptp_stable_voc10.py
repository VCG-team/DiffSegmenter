...
def get_args_parser():
    ...
    return parser
#数据集中的类别
embs = ['airplane', 'bag', 'bed', 'bedclothes', 'bench', 'bicycle', 'bird', 'boat', 'book',
...
if __name__ == '__main__':
    ...
    args.img_list='dataset/voc10'
    #your data path
    args.data_path='/VOCdevkit/VOC2010'
    ...
    # pred_classes=read_json(json_path,choice=['text_similarity','image_similarity'])
                    ...
                    for idx,res in enumerate([8,16,32,64]):
                        try:
                            if prompt[0].split(" ")[3+word_len].endswith("ing"):
                                cross_att = cross_attention_maps[idx][:,:,[3+embs_len,4+embs_len]].mean(2).view(res,res).float()
